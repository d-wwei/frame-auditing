...
with open("cognitive-protocol.md") as f:
    frame_auditing_rules = f.read()

model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction=frame_auditing_rules
)
```
...
Place Frame Auditing first in the system instruction, then First Principles, then Tacit Knowledge. All are concise enough to fit together without significant context cost.
